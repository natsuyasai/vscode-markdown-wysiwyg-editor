...
1. 対象コンポーネントの既存Storyを確認する (`webview-ui/src/components/<Name>/<Name>.stories.tsx`)
...
2. Story（`webview-ui/src/components/<Name>/<Name>.stories.tsx`）を先に完成させる
...
- `webview-ui/src/components/ContextMenu/ContextMenu.stories.tsx` — play function・テーマバリエーションの参考実装
- `webview-ui/src/App.*.stories.tsx` — play functionによる画面結合テストの参考実装。共通処理は `webview-ui/src/App.testUtils.ts` に集約
