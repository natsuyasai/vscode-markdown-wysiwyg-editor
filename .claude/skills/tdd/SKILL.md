...
テスト・Storyは対象ソースと**同じディレクトリ**に隣り合わせて配置する。

- コンポーネントテスト: `webview-ui/src/components/<Name>/<Name>.spec.tsx`
- hooksテスト: `webview-ui/src/hooks/<hookName>.spec.ts`
- ユーティリティテスト: `webview-ui/src/utilities/<utilName>.spec.ts`
- Storybookテスト: `webview-ui/src/components/<Name>/<Name>.stories.tsx`（play function）
